...
!   Wtilde is the piecewise-linear W'' companion (flat to 3/4, join 4/3)
...
!
 ...
 real, parameter, public  :: cnormk_tilde = 25920./(130009.*pi)
 real, parameter, public  :: wab0_tilde = 1., gradh0_tilde = -3.*wab0_tilde
...
pure subroutine get_kernel_tilde(q2,q,wkern,grkern)
 ...
 !--number-density companion: W''=0 on [0,3/4], well at q=4/3
 if (q < 0.75) then
    wkern  = 1.
    grkern = 0.
 elseif (q < 4./3.) then
    wkern  = (-192.*q2*q + 432.*q2 - 324.*q + 256.)/175.
    grkern = (-576.*q2 + 864.*q - 324.)/175.
 elseif (q < 2.) then
    wkern  = (69.*q2*q - 324.*q2 + 468.*q - 192.)/25.
    grkern = (207.*q2 - 648.*q + 468.)/25.
 else
    wkern  = 0.
    grkern = 0.
 endif
