...
#---------------------------------
def print_wkern_grkern(w,dw,label):
    """Print the if/elseif body that sets wkern and grkern from W and W'."""
    print_decl(w)
    print (" !--%s" %label)
    if isinstance(w, Piecewise):
       for i, (e, c) in enumerate(w.args):
...
def printkernel_phantom(w,R,name,wtilde=None):
    import datetime
    dw, d2w, c1D, c2D, c3D, fsoft, pot, dpotdh, gsoft = getkernelfuncs(w,R)
    w0 = w.subs(q,0)
    ...
    if wtilde is not None:
       print ("!   Wtilde is the piecewise-linear W'' companion (flat to 3/4, join 4/3)")
    ...
    if wtilde is None:
       ...
       c1t, c2t, c3t = getnorm(wtilde,R)
       w0t = wtilde.subs(q,0)
       print (" real, parameter, public  :: cnormk_tilde = %s" %fmt(c3t))
       print (" real, parameter, public  :: wab0_tilde = %s, gradh0_tilde = -3.*wab0_tilde" %fmt(w0t))
    ...
    if wtilde is None:
       ...
       dwt = diff(wtilde,q)
       print_wkern_grkern(wtilde,dwt,'number-density companion')
...
#----------------------------------------------------------------------
# Piecewise-linear W'' companion (flat core, simple-fraction knots)
#
#   W'' = 0 on [0, 3/4]
#   W'' linear to a well at q = 4/3
#   W'' linear to q = R with W''(R) fixed by W'(R)=0
#   W(0)=1, W(R)=0;  3D norm is 25920/(130009 pi)
#----------------------------------------------------------------------
def wpp_pwl_flat43(R):
    w_mid = (Rational(-192,175)*q**3 + Rational(432,175)*q**2
             - Rational(324,175)*q + Rational(256,175))
    w_out = (Rational(69,25)*q**3 - Rational(324,25)*q**2
             + Rational(468,25)*q - Rational(192,25))
    f = Piecewise((1, q < Rational(3,4)),
                  (w_mid, q < Rational(4,3)),
                  (w_out, q < R),
                  (0, True))
    return (f, 'Wpp-flat 3/4 join 4/3')
...
f, name = m4(R)
#f, name = w6(R)
# number-density companion (Wtilde); None => same as W
ftilde, _ = wpp_pwl_flat43(R)
...
printkernel_phantom(f,R,name,wtilde=ftilde)
#plotkernel(f,R,name)
